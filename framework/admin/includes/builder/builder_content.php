...
function ppb_header_func($atts, $content) {
	...
	if(!empty($title))
	{
		$return_html.= '<h2 class="ppb_title" style="'.esc_attr($custom_css_fontcolor).'">'.esc_html(urldecode($title)).'</h2>';
		$return_html.= '<div class="or-spacer '.esc_attr($textalign).'">
		  <div class="mask"></div>
		  <span></span>
		</div>';
	}
	...
	if(!empty($subtitle))
	{
		$return_html.= '<div class="ppb_subtitle" style="'.esc_attr($custom_css_fontcolor).'">'.esc_html(urldecode($subtitle)).'</div>';
	}
	
	if(!empty($content))
	{
		$return_html.= '<br class="clear"/>'.do_shortcode(tg_apply_content($content));
	}
...
function ppb_gallery_grid_func($atts, $content) {
	...
	$return_html = '<div '.$sec_id.' class="'.esc_attr($size).' nopadding" style="'.$custom_css.'">';
...
function ppb_animated_gallery_grid_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		...
		{
			$return_html.= '<div class="ri-grid-overlay">';
			
			$return_html.= '<div class="ri-grid-wrapper"><div class="ri-grid-item"><div class="ri-grid-border" '.$custom_border_css.'><div class="ri-grid-content" '.$custom_bg_css.'>';
			if(empty($logo) && !empty($title))
			{
				$return_html.= '<h1 '.$custom_font_css.'>'.urldecode($title).'</h1>';
			}
			else if(!empty($logo))
			{
				$return_html.= '<img src="'.esc_url($logo).'" class="grid-logo" alt=""/>';
			}
			
			if(!empty($subtitle))
			{
		    	$return_html.= '<div class="or-spacer">
				  <div class="mask"></div>
				  <span></span>
				</div>';
			
	    	    $return_html.= '<div class="page_tagline grid" '.$custom_font_css.'>'.urldecode($subtitle).'</div>';
	    	}
...
function ppb_image_half_fullwidth_func($atts, $content) {
	...
	if($align=='left')
	{
		...
		if(!empty($title))
		{
			...
			$return_html.= '>'.esc_html(urldecode($title)).'</h2>';
			$return_html.= '<div class="or-spacer '.esc_attr($textalign).'">
			  <div class="mask"></div>
			  <span></span>
			</div>';
		...
		if(!empty($subtitle))
		{
			...
			$return_html.= '>'.esc_html(urldecode($subtitle)).'</div>';
		}
		if(!empty($content))
		{
...
function ppb_blog_grid_func($atts, $content) {
	...
	if(!empty($posts_arr) && is_array($posts_arr))
	{
		...
		foreach($posts_arr as $key => $ppb_post)
		{
			...
			$return_html.= '<div class="post_grid_excerpt">'.pp_substr(strip_shortcodes(pp_get_the_excerpt($ppb_post->ID)), 150).'</div>';
...
function ppb_contact_map_func($atts, $content) {
	...
	if(!empty($content))
	{
		$return_html.= '<br class="clear"/><div class="ppb_content">'.urldecode($content).'</div>';
	}
    ...
    if(is_array($pp_contact_form) && !empty($pp_contact_form))
    {
        foreach($pp_contact_form as $form_input)
        {
        	switch($form_input)
        	{
    				...
    				<textarea id="message" name="message" rows="7" cols="10" class="required_field" placeholder="'.__( 'Message *', THEMEDOMAIN ).'"></textarea>
    				';	
    ...
    $return_html.= '<br/><br/><div class="contact_submit_wrapper">
    	<input id="contact_submit_btn'.$custom_id.'" name="contact_submit_btn'.$custom_id.'" type="submit" class="solidbg" value="'.__( 'Send Message', THEMEDOMAIN ).'" ';
...
function ppb_contact_sidebar_func($atts, $content) {
	...
	if(!empty($title))
	{
		$return_html.= '<h2 class="ppb_title">'.esc_html(urldecode($title)).'</h2>';
		$return_html.= '<div class="or-spacer">
		  <div class="mask"></div>
		  <span></span>
		</div>';
	...
	if(!empty($subtitle))
	{
		$return_html.= '<div class="ppb_subtitle">'.esc_html(urldecode($subtitle)).'</div>';
	}
...
function ppb_galleries_func($atts, $content) {
	...
	$return_html = '<div class="'.esc_attr($size).' ppb_galleries nopadding" ';
