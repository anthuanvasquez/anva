...
class TG_Nav_Menu_Item_Custom_Fields {
	...
	static function setup() {
		...
		$current_sidebars = anva_get_sidebar_locations();
		$mega_menu_area_arr = array();
		
		if(!empty($current_sidebars))
		{
			foreach( $current_sidebars as $key => $sidebar ) {
				$mega_menu_area_arr[$key] = $sidebar['args']['name'];
			}
		}
		...
		self::$options['fields'] = array(
			'mega_menu' => array(
				'name' => 'mega_menu',
				'label' => __('Use Mega Menu', anva_textdomain() ),
				'container_class' => '',
				'input_type' => 'checkbox',
			),
			'mega_menu_sidebar' => array(
				'name' => 'mega_menu_sidebar',
				'label' => __('Display Widget Area', anva_textdomain() ),
				'container_class' => '',
				'input_type' => 'select',
...
class tg_walker extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if(!empty($use_mega_menu))
		{
			...
			$item_output.= '<div class="mega_menu_wrapper"><div class="mega_menu_bg"><ul class="sidebar_widget">'.get_dynamic_sidebar($mega_menu_sidebar).'</ul></div></div>';
		}
